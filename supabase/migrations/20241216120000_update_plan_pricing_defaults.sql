-- Update plan pricing defaults (monthly_fee) and keep override-preserving logic on plan change.
-- New defaults: Starter 99, Basic 399, Premium 699, Enterprise 0, Custom 2499.
...
-- Backfill monthly_fee for rows still at old defaults (basic 299, premium 2499, custom 0)
UPDATE public.workspaces
SET monthly_fee = CASE plan
  WHEN 'basic' THEN 399
  WHEN 'premium' THEN 699
  WHEN 'custom' THEN 2499
  WHEN 'starter' THEN 99
  ...
  ELSE monthly_fee
END
WHERE (plan = 'basic' AND monthly_fee = 299)
   OR (plan = 'premium' AND monthly_fee = 2499)
   OR (plan = 'custom' AND monthly_fee = 0);
